  ...
  return (
    ...
    >
      ...
      <g id="tier-row-headers">
        <g transform="translate(5, 80)" className={onSelectSlot ? "cursor-pointer" : ""} onClick={() => onSelectSlot && selectedBay && selectedSlot && onSelectSlot(selectedBay, 3, selectedSlot)}>
        ...
        <g transform="translate(5, 205)" className={onSelectSlot ? "cursor-pointer" : ""} onClick={() => onSelectSlot && selectedBay && selectedSlot && onSelectSlot(selectedBay, 2, selectedSlot)}>
        ...
        <g transform="translate(5, 330)" className={onSelectSlot ? "cursor-pointer" : ""} onClick={() => onSelectSlot && selectedBay && selectedSlot && onSelectSlot(selectedBay, 1, selectedSlot)}>
      ...
      <g id="storage-containers-layer">
        {[3, 2, 1].map((tier) => {
          const tierYOffset = tier === 3 ? 65 : tier === 2 ? 190 : 315;
          ...
          return bayNumbers.map((bay, bIdx) => {
            ...
            return [...Array(itemsPerBay)].map((_, slotIdx) => {
              ...
              return (
                ...
                >
                  {isSlotSelected && (
                    <rect x="-4" y="-4" width={containerWidth + 8} height="120" fill="none" stroke="#06b6d4" strokeWidth="3" strokeDasharray="6 4" rx="4" filter="url(#targetGlow)" />
                  )}
                  ...
                  {storageType === 'bins' && (
                    <g id={`bin-${bay}-${tier}-${slotNum}`}>
                      ...
                      <rect x="2" y="18" width={containerWidth - 4} height="85" rx="4" fill={customTierHex} stroke="#0f172a" strokeWidth="1.2" filter="url(#cadShadow)" />
                      <polygon points={`${containerWidth - 2},18 ${containerWidth + 8},0 ${containerWidth + 8},85 ${containerWidth - 2},103`} fill={customTierHex} stroke="#0f172a" strokeWidth="0.8" opacity="0.75" />
                      <rect x="4" y="24" width={containerWidth - 8} height="12" rx="2" fill="#ffffff" opacity="0.15" />
                      <line x1="8" y1="90" x2={containerWidth - 8} y2="90" stroke="#0f172a" strokeWidth="2" opacity="0.3" strokeLinecap="round" />
                      
                      {containerWidth >= 24 && (() => {
                        const slotLabelText = getSlotLabel(bay, tier, slotNum);
                        const badgeW = Math.max(34, Math.min(containerWidth - 8, slotLabelText.length * (tagFontSize * 0.72) + 8));
                        const badgeH = Math.max(22, tagFontSize + 8);
                        const badgeX = containerWidth / 2 - badgeW / 2;
                        const badgeY = 46 - (tagFontSize - 10) / 2;
                        ...
                        return (
                          ...
                          >
                            <rect x={badgeX} y={badgeY} width={badgeW} height={badgeH} rx="4" fill="#ffffff" stroke="#0f172a" strokeWidth="1.5" filter="url(#cadShadow)" />
                            <text x={containerWidth / 2} y={badgeY + badgeH / 2 + (tagFontSize * 0.35)} fill="#0f172a" fontSize={tagFontSize} fontWeight="900" textAnchor="middle" fontFamily="monospace, sans-serif">
                              {slotLabelText}
                            </text>
                          </g>
                        );
                      ...
                      {containerWidth >= 35 && item && (
                        <text x={containerWidth / 2} y="82" fill="#60a5fa" fontSize="9" fontWeight="900" textAnchor="middle">
                    ...
                    <g id={`pallet-${bay}-${tier}-${slotNum}`}>
                      ...
                      {containerWidth >= 24 && (() => {
                        ...
                        return (
                          ...
                          >
                            <rect x={badgeX - 2} y={badgeY - 2} width={badgeW + 4} height={badgeH + 4} rx="7" fill="#f59e0b" opacity="0" className="group-hover/label:opacity-90 transition-opacity" />
                            <rect x={badgeX} y={badgeY} width={badgeW} height={badgeH} rx="5" fill="#ffffff" stroke="#78350f" strokeWidth="1.8" filter="url(#cadShadow)" />
                            <text x={containerWidth / 2} y={badgeY + badgeH / 2 + (tagFontSize * 0.35)} fill="#0f172a" fontSize={tagFontSize} fontWeight="900" textAnchor="middle" fontFamily="monospace, sans-serif">
                              {slotLabelText}
                            </text>
                          </g>
                      ...
                      {containerWidth >= 40 && item && (
                        <text x={containerWidth / 2} y="64" fill="#ffffff" fontSize="8.5" fontWeight="900" textAnchor="middle">
                          {item.quantity} {item.unit || 'cái'}
                        </text>
                      )}
                    </g>
                  ...
                  {storageType === 'cartons' && (
                    <g id={`carton-${bay}-${tier}-${slotNum}`}>
                      ...
                      <rect x="2" y="10" width={containerWidth - 4} height="85" rx="3" fill="#f8fafc" stroke="#64748b" strokeWidth="1.2" filter="url(#cadShadow)" />
                      <polygon points={`${containerWidth - 2},10 ${containerWidth + 6},0 ${containerWidth + 6},85 ${containerWidth - 2},95`} fill="#cbd5e1" stroke="#64748b" strokeWidth="0.8" />
                      <line x1={(containerWidth - 4) / 2 + 2} y1="10" x2={(containerWidth - 4) / 2 + 2} y2="95" stroke="#94a3b8" strokeWidth="2.5" opacity="0.6" strokeDasharray="5 2" />
                      
                      <rect x="15" y="25" width={containerWidth - 30} height="40" fill={customTierHex} stroke="#ffffff" strokeWidth="2" filter="url(#cadShadow)" opacity="0.8" />
                      
                      {containerWidth >= 24 && (() => {
                        const slotLabelText = getSlotLabel(bay, tier, slotNum);
                        const badgeW = Math.max(30, Math.min(containerWidth - 10, slotLabelText.length * (tagFontSize * 0.72) + 8));
                        const badgeH = Math.max(20, tagFontSize + 8);
                        const badgeX = containerWidth / 2 - badgeW / 2;
                        const badgeY = 35 - (tagFontSize - 10) / 2;
                        ...
                        return (
                          ...
                          >
                            <rect x={badgeX} y={badgeY} width={badgeW} height={badgeH} rx="3" fill="#ffffff" stroke="#64748b" strokeWidth="1.2" />
                            <text x={containerWidth / 2} y={badgeY + badgeH / 2 + (tagFontSize * 0.35)} fill="#0f172a" fontSize={tagFontSize} fontWeight="900" textAnchor="middle" fontFamily="monospace, sans-serif">
                              {slotLabelText}
                            </text>
                          </g>
                      ...
                      {containerWidth >= 35 && item && (
                        <text x={containerWidth / 2} y="85" fill="#334155" fontSize="8.5" fontWeight="900" textAnchor="middle">
                          {item.quantity} {item.unit || 'cái'}
                        </text>
                      )}
                    </g>
